...
unsigned short ktv_int16WithBytes(Byte * bytes);

@end

@implementation KTVVideoACVFile
...
- (instancetype)initWithACVFileData:(NSData *)data
{
    if (self = [super init])
    {
        ...
        version = ktv_int16WithBytes(rawBytes);
        rawBytes += 2;
        
        totalCurves = ktv_int16WithBytes(rawBytes);
        rawBytes += 2;
        ...
        {
            unsigned short pointCount = ktv_int16WithBytes(rawBytes);
            rawBytes += 2;
            ...
            {
                unsigned short y = ktv_int16WithBytes(rawBytes);
                rawBytes += 2;
                unsigned short x = ktv_int16WithBytes(rawBytes);
                rawBytes += 2;
...
@interface KTVVPFilterToneCurve ()
...
@property (nonatomic, strong) KTVVPGLToneCurveProgram * glProgram;
@property (nonatomic, strong) KTVVPGLPlaneModel * glModel;

@end

@implementation KTVVPFilterToneCurve
...
- (instancetype)initWithContext:(KTVVPContext *)context
{
    if (self = [super initWithContext:context])
    {
        ...
        _glProgram = [[KTVVPGLToneCurveProgram alloc] init];
        _glModel = [[KTVVPGLPlaneModel alloc] init];
        
        NSURL * fileURL = [[NSBundle mainBundle] URLForResource:@"rixi" withExtension:@"acv"];
        NSData * fileData = [NSData dataWithContentsOfURL:fileURL];
        
        KTVVideoACVFile * curve = [[KTVVideoACVFile alloc] initWithACVFileData:fileData];
...
- (void)putFrame:(KTVVPFrame *)frame
{
    ...
    [_glProgram bindInputImageTexture:frame.texture
